    ...
    public partial class Trade_Auto_Daishin : Form
    {
        ...
        private void Match_Click(object sender, EventArgs e)
        {
            ...
            if (findRows.Any())
            {
                ...
                {
                    DataRow[] findRows2 = dtCondStock_Transaction.AsEnumerable().Where(row => row.Field<string>("종목번호") == findRows[i]["종목번호"].ToString() && row.Field<string>("구분코드") == findRows[i]["구분코드"].ToString() && row.Field<string>("매매구분") == "매수" && row.Field<string>("체결단가") != "0").ToArray();
                    if (findRows2.Any())
                    {
                        findRows[i]["상태"] = "매수완료";
                        findRows[i]["편입상태"] = "실매입";
            ...
            DataRow[] findRows3 = dtCondStock.AsEnumerable().Where(row => row.Field<string>("상태").Trim() == "매도중" && row.Field<string>("보유수량").Trim() == "0/0").ToArray();
            if (findRows3.Any())
            {
                for (int i = 0; i < findRows3.Length; i++)
                {
                    DataRow[] findRows4 = dtCondStock_Transaction.AsEnumerable().Where(row => row.Field<string>("주문번호") == findRows3[i]["주문번호"].ToString() && row.Field<string>("구분코드") == findRows[i]["구분코드"].ToString() && row.Field<string>("매매구분") == "매도" && row.Field<string>("체결단가") != "0").ToArray();
                    if (findRows4.Any())
                    {
                        findRows3[i]["상태"] = "매도완료";
        ...
        private int condition_sub_code(string condition_code)
        {
            ...
            while (true)
            {
                ...
                else if (CssWatchStgSubscribe.GetDibStatus() == 1)
                {
                    check++;
        ...
        private void GetCashInfo(string acc_gubun, bool update)
        {
            if (TradeInit())
            {
                ...
                while (true)
                {
                    ...
                    else if (CpTd6033.GetDibStatus() == 1)
                    {
                        check++;
                ...
                {
                    WriteLog_System($"[예수금및계좌보유/수신실패] : {error_message(result)} / {CpTd6033.GetDibMsg1()}\n");
                }
            }
        }
        ...
        private void today_profit_tax_load(string load_type, string acc_gubun)
        {
            ...
            if (TradeInit())
            {
                ...
                while (true)
                {
                    ...
                    else if (CpTd6032.GetDibStatus() == 1)
                    {
                        check++;
        ...
        {
            WriteLog_System($"[체결내역업데이트/{gubun}/{order_number}] : 접속 시도\n");
            ...
            while (true)
            {
                ...
                else if (CpTd5341.GetDibStatus() == 1)
                {
                    check++;
            ...
            if (result == 0)
            {
                ...
                if (count == 0)
                {
                    WriteLog_System($"[체결내역업데이트/{gubun}/{order_number}] : 실매입 가격 업데이트(체결내역없음)\n");
                    return;
                }

                WriteLog_System($"[체결내역업데이트/{gubun}/{order_number}] : 성공\n");
            ...
            {
                WriteLog_System($"[체결내역/수신실패] : {error_message(result)} / {CpTd5341.GetDibMsg1()}\n");
            }
        }
            ...
            */
            ...
            if (utility.dow_index)
            {
                ...
                while (true)
                {
                    ...
                    else if (CpFore8312.GetDibStatus() == 1)
                    {
                        check++;
        ...
        private void index_stop_skip(string date)
        {
            if (utility.Foreign_Stop || utility.Foreign_Skip)
            {
                ...
                {
                    WriteLog_System("시스템 시간 한국어 변경 요망\n");
                    return;
                }
                //
                if (DateTime.TryParseExact(date, "yyyyMMdd", null, System.Globalization.DateTimeStyles.None, out DateTime givenDate))
                {
                    ...
                    //표기
                    WriteLog_System($"EDT시각{givenDate.ToString()} / KOR시각{currentDate.ToString()}\n");

                    //날짜 차이 계산
                    TimeSpan difference = currentDate.Date - givenDate.Date;
        ...
        private void KOR_INDEX()
        {
            ...
            if (utility.Foreign_commodity)
            {
                ...
                while (true)
                {
                    ...
                    else if (CpSvrNew7224.GetDibStatus() == 1)
                    {
                        check++;
            ...
            if (utility.kospi_commodity)
            {
                ...
                while (true)
                {
                    ...
                    else if (FutOptChart.GetDibStatus() == 1)
                    {
                        check++;
            ...
            if (utility.kosdak_commodity)
            {
                ...
                while (true)
                {
                    ...
                    {
                        check++;
                        WriteLog_System("[DibRq요청/수신대기/300ms] : KOSDAK150\n");
                        System.Threading.Thread.Sleep(delay1);
                    }
                    else
                    {
        ...
        private void Condition_load()
        {
            ...
            while (true)
            {
                ...
                else if (CssStgList.GetDibStatus() == 1)
                {
                    check++;
                    WriteLog_System("[DibRq요청/수신대기/300ms] : 조건식 조회\n");
                    System.Threading.Thread.Sleep(delay1);
                }
                else
                {
        ...
        private void normal_search(object sender, EventArgs e)
        {
            ...
            while (true)
            {
                ...
                else if (CssWatchStgControl.GetDibStatus() == 1)
                {
                    check++;
                    WriteLog_System("[DibRq요청/수신대기/300ms] : 매도 전용 조건식 검색\n");
                    System.Threading.Thread.Sleep(delay1);
                }
                else
                {
        ...
        private void real_time_search(object sender, EventArgs e)
        {
            ...
            foreach (string Fomula in utility.Fomula_list_buy_text.Split(','))
            {
                ...
                while (true)
                {
                    ...
                    {
                        check++;
                        WriteLog_System("[DibRq요청/수신대기/500ms] : 매수 전용 조건식 검색\n");
        ...
        private bool stock_initial(string condition_code, string condition_name)
        {
            ...
            if (result == 0)
            {
                ...
                WriteLog_Stock("[실시간조건식/시작/" + condition_name + "] : 초기 검색 종목 정보 끝\n");
                telegram_message("[실시간조건식/시작/" + condition_name + "] : 초기 검색 종목 정보 끝\n");
            }
            else
            {
        ...
        private void Stock_info(string condition_name, string Code, string hold_num, string order_number, string gubun)
        {
            ...
            while (true)
            {
                ...
                else if (MarketEye.GetDibStatus() == 1)
                {
                    check++;
                    WriteLog_System($"[{condition_name}/수신재시도] : ({Code}) 5초 대기 후 최대 5회 재시도\n");
                    System.Threading.Thread.Sleep(5000);
                }
                else
                {
            ...
            if (result == 0)
            {
                ...
                //최소 및 최대 매수가 확인
                if (condition_name != "HTS매매" && condition_name != "전일보유" && (Native_Price < Convert.ToInt32(utility.min_price) || Native_Price > Convert.ToInt32(utility.max_price)))
                {
                ...
                //초기검색 항목 점검용
                if (findRows.Any() && utility.buy_OR)
                {
                    WriteLog_Stock($"[전일보유/{condition_name}/편입실패] : {Code_name}({Code}) OR 모드 중복\n");
                    return;
                }

                if (findRows.Any() && utility.buy_AND && condition_name != Convert.ToString(findRows[0]["조건식"]))
                {
                ...
                //보유종목 평균가 받기
                DataRow[] findRows2 = dtCondStock_hold.Select($"종목코드 = '{Code}'");
                ...
                dtCondStock.Rows.Add(
                    ...
                    condition_name == "HTS매매" || condition_name == "전일보유" ? findRows2[0]["평균단가"].ToString() : Current_Price,
                    "-",
                    "0.00%",
                    hold_num + "/" + now_hold,
                    condition_name,
                    ...
                    if (findRows[i]["상태"].Equals("매수완료") && !percent.Equals(""))
                    {
                        lock (sell_lock)
                        {
                            ...
                            if (!sell_runningCodes.ContainsKey(order_num))
                            {
                                ...
                                {
                                    sell_check_price(price.Equals("") ? findRows[i]["현재가"].ToString() : string.Format("{0:#,##0}", Convert.ToInt32(price)), percent, Convert.ToInt32(findRows[i]["보유수량"].ToString().Split('/')[0]), Convert.ToInt32(findRows[i]["편입가"].ToString().Replace(",", "")), order_num, findRows[i]["구분코드"].ToString(), 0);
                                }
        ...
        private void stock_in_out()
        {
            ...
            switch (gubun)
            {
                ...
                case "49":
                    ...
                    else if (utility.buy_INDEPENDENT)
                    {
                        ...
                        else if (findRows1.Length == 1)
                        {
                            ...
                            {
                                WriteLog_Stock($"[기존종목/INDEPENDENT편입/{Condition_Name}] : {findRows1[0]["종목명"]}({Stock_Code}) 전일 보유 종목 \n");
                            }
                ...
                case "50":
                    ...
                    WriteLog_Stock($"[기존종목/이탈/{Condition_Name}] : {findRows[0]["종목명"]}({Stock_Code}) 재이탈 대상 없음\n");
        ...
        //timer2(300ms) : 편입된 종목에 대하여 매수 및 청산 확인
        ...
        private string buy_check(string code, string code_name, string price, string time, string high, bool check, string condition_name, string gubun)
        {
            ...
            if (order_method[0].Equals("시장가"))
            {
                ...
                while (true)
                {
                    ...
                    else if (CpTd0311.GetDibStatus() == 1)
                    {
                        check2++;
        ...
        //실시간 가격 매도
        private void sell_check_price(string price, string percent, int hold, int buy_price, string order_num, string gubun, double down_percent)
        {
            //익절
            ...
            if (findRows.Any())
            {
                ...
                if (market_time == 1)
                {
                    ...
                    while (true)
                    {
                        ...
                        else if (CpTd0322.GetDibStatus() == 1)
                        {
                            check2++;
                ...
                else if (market_time == 2)
                {
                    ...
                    while (true)
                    {
                        ...
                        else if (CpTd0386.GetDibStatus() == 1)
                        {
                            check2++;
        ...
        private void Trade_Check()
        {
            ...
            if (cancel == "3" && gugu == "2") //최소주문 && 확인
            {
                Trade_check_save.Enqueue(tmp);
            }
            else if (findRows.Any())
            {
            ...
            else
            {
                ...
                if (cancel.Equals("1") && trade_Gubun.Equals("매수"))
                {
                    ...
                    {
                        WriteLog_System($"[체결내역] 주문번호 없음 - {tmp[3]} 재시작 요망\n");
                    }
                }
                else if (cancel.Equals("1") && trade_Gubun.Equals("매도") && hold_sum.Equals("0"))
                {
        ...
        private void order_close(string trade_type, string order_number, string gubun, string code_name, string code, string order_acc)
        {
            ...
            if (market_time == 1)
            {
                ...
                while (true)
                {
                    ...
                    else if (CpTd0326.GetDibStatus() == 1)
                    {
                        check2++;
            ...
            else if (market_time == 2)
            {
                ...
                while (true)
                {
                    ...
                    else if (CpTd0387.GetDibStatus() == 1)
                    {
                        check2++;
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    else if (CpTd0314.GetDibStatus() == 1)
                    {
                        check2++;
                ...
                {
                    string order_number_cancel = Convert.ToString(CpTd0314.GetHeaderValue(6).ToString());
                    string order_number_cancel2 = Convert.ToString(CpTd0314.GetHeaderValue(1).ToString());
        ...
        */
        ...
        public async Task TradingVIew_Listener_Start()
        {
            _listener.Start();
